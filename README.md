...
### 代码层面
 ...
 4. const > let const有助于内存回收 与 命名不重复
 5. 闭包（匿名函数）不可乱用，否则导致内存泄漏，从而使组件无法销毁，造成内存负担
 6. 使用`hasOwnProperty` 与 `path` 对象的路径假设不存在的对象返回false,避免代码报错
 7. 空对象 / 固定数据仅仅定义一次,根据需要引用。而不胡乱使用内存
 8. 参考学习源代码
