<template>
  <section class="about">
    ...
    <el-select v-model="value1">
      <el-option :class="{'danger':item.id == -1}" v-for="(item, index) in useNativeAPIGetData" :key="index" :value="item.id">{{item}}</el-option>
    </el-select>

    <h4>UER VUEX GET DATA ( 使用VUEX方式获取数据 )</h4>
    <el-select v-model="value2">
      <el-option :class="{'danger':item.id == -1}" v-for="(item, index) in userList" :key="index" :value="item.id">{{item}}</el-option>
    </el-select>
  </section>
</template>
<script>
