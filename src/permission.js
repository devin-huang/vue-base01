...
router.beforeEach(async (to, from, next) => {
  ...
  if (to.query.po_session_key) {
    ...
    if (+store.getters.addRouters !== 0 && +store.state.Permission.permission !== 0) {
      return next()
    } else {
      const { error, data } = await listPoMenu()
      if (error) return next(false)
      // 渲染菜单
      const addRouters = await store.dispatch('Permission/GenerateRouters', data, { root: true }) // Permission是指store.js的Permission模块
