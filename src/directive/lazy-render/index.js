...
let initScrollInfo = function(scrollTop = 0) {
  fixedTbody = document.querySelectorAll(
    '[class|=el-table__fixed] .el-table__body tbody'
  )
  fixedBodyWrapper = document.querySelectorAll(
    '[class|=el-table__fixed] .el-table__fixed-body-wrapper'
  )
...
let initDefine = function(el, binding, vnode, oldVnode, status = true) {
  ...
  oldDataSize =
    Object.keys(oldVnode.data).length > 0 && oldVnode.data.attrs['data-size']
...
let appendTrElement = function() {
  const hasOpenHeight =
    selectTbody && selectTbody.querySelectorAll('.openHeight').length
  if (hasOpenHeight) {
...
let scrollControlFixedColumn = function(scrollTop) {
  if (createElementTRHeight - topPx <= 0) {
    fixedTbody.forEach(e => {
      e.setAttribute(
        'style',
        `transform: translateY(${-firstTRchild.clientHeight +
          (createElementTRHeight - topPx)}px)`
      )
    })
    ...
    fixedTbody.forEach(e => {
      e.setAttribute(
        'style',
        `transform: translateY(${-firstTRchild.clientHeight}px)`
      )
    })
  }
}
        ...
        selectWrap.addEventListener('scroll', function() {
          ...
          let top =
            createElementTRHeight - topPx > 0
              ...
              : 0
          let transformTop =
            createElementTRHeight - topPx > 0 ? topPx : createElementTRHeight
          createElementTR.setAttribute('style', `height: ${top}px;`)
